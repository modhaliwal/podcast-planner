...
export const SelectedGuestsGrid = memo(function SelectedGuestsGrid({
  ...
  const validGuests = useMemo(() => selectedGuestIds.map(guestId => availableGuests.find(g => g.id === guestId)).filter((guest): guest is Guest => guest !== undefined), [selectedGuestIds, availableGuests]);
  ...
  return <div className="mt-4 space-y-3">
      ...
      <div key={guest.id} className="flex items-start bg-card border rounded-md group relative">
            <GuestChip guest={guest} size="lg" showLink={false} className="flex-1 min-w-0 bg-transparent hover:bg-transparent" />
            
            <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 rounded-full opacity-0 group-hover:opacity-100 absolute top-1 right-1" onClick={() => onRemoveGuest(guest.id)}>
              ×
            </Button>
          </div>)}
      </div>
